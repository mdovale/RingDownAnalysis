...
def profile_single_file_analysis(
    ...
    if filepath is None:
        ...
        signal = RingDownSignal(
            f0=5.0, fs=100.0, N=N, A0=1.0, snr_db=60.0, Q=10000.0
        )
...
def profile_batch_analysis(
    ...
    try:
        signal = RingDownSignal(
            f0=5.0, fs=100.0, N=N_per_file, A0=1.0, snr_db=60.0, Q=10000.0
        )
        rng = np.random.default_rng(42)
        ...
        for i in range(n_files):
            ...
            with open(filepath, 'w') as f:
                for ti, xi in zip(t[::max(1, N_per_file//10000)], x[::max(1, N_per_file//10000)]):
                    f.write(f"{ti:.6f},0,0,{xi:.6f}\n")
...
def print_bottleneck_report(analysis: Dict):
    ...
    if analysis['scipy_functions']:
        ...
        print(f"Total time in scipy: {scipy_time:.4f} s ({scipy_time/analysis['total_time']*100:.1f}%)")
    ...
    if analysis['numpy_functions']:
        ...
        print(f"Total time in numpy: {numpy_time:.4f} s ({numpy_time/analysis['total_time']*100:.1f}%)")
    ...
    if analysis['ringdown_functions']:
        ...
        print(f"Total time in ringdown: {ringdown_time:.4f} s ({ringdown_time/analysis['total_time']*100:.1f}%)")
