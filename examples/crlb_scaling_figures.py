...
def plot_frequency_crlb_vs_tau_ratio(params=None, ax=None, figsize=None, dpi=None, *args, **kwargs):
    ...
    *args, **kwargs
    ...
    ax.loglog(tau_ratios[valid], crlb_f[valid], 'b-', linewidth=2, label='Exact CRLB', *args, **kwargs)
    ax.loglog(tau_ratios, crlb_slow_decay, 'r--', linewidth=1.5, alpha=0.7, 
              label='Slow-decay approximation ($T \\ll \\tau$)')
    ax.axhline(crlb_rapid_decay_value, color='lime', linestyle='--', linewidth=1.5, 
               alpha=0.7, label='Rapid-decay approximation ($T \\gg \\tau$)')
...
def plot_frequency_crlb_vs_snr(params=None, ax=None, figsize=None, dpi=None, *args, **kwargs):
    ...
    *args, **kwargs
    ...
    # Filter out NaN values for plotting
    valid = ~np.isnan(crlb_f)
    ax.semilogy(snr_db_values[valid], crlb_f[valid], 'b-', linewidth=2, label='Exact CRLB', *args, **kwargs)
    ax.semilogy(snr_db_values, theoretical, 'r--', linewidth=1.5, alpha=0.7,
...
def plot_q_crlb_vs_q(params=None, ax=None, figsize=None, dpi=None, *args, **kwargs):
    ...
    *args, **kwargs
    ...
    ax.loglog(Q_values[valid], rel_error[valid], 'b-', linewidth=2, label='Exact CRLB', *args, **kwargs)
    
    # Plot high-Q scaling only where valid
    high_q_valid = ~np.isnan(high_q_scaling)
    if np.any(high_q_valid):
        ax.loglog(Q_values[high_q_valid], high_q_scaling[high_q_valid], 
...
def plot_q_crlb_vs_tau_ratio(params=None, ax=None, figsize=None, dpi=None, *args, **kwargs):
    ...
    *args, **kwargs
    ...
    # Filter out NaN values for plotting
    valid = ~np.isnan(rel_error)
    ax.loglog(tau_ratios[valid], rel_error[valid], 'b-', linewidth=2, label='Exact CRLB', *args, **kwargs)
    ...
    ax.text(1.0, rel_error.min() * 1.5, '$T = \\tau$', 
