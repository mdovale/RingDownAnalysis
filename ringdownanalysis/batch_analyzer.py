...
class BatchRingDownAnalyzer:
            ...
            for filepath in filepaths:
                try:
                    ...
                    if verbose:
                        ...
                        print(f"  Cropped to: {result['T_crop']:.2f} s "
    ...
    def consistency_analysis(self) -> Dict:
        ...
        with open('/Users/mdovale/Work-local/RingDownAnalysis/.cursor/debug.log', 'a') as f:
            import json, time
            f.write(json.dumps({'sessionId':'debug-session','runId':'initial','hypothesisId':'A,B,C,D,E','location':'batch_analyzer.py:214','message':'consistency_analysis entry','data':{'n_results':len(self.results),'results_f_nls':[r.get('f_nls') for r in self.results] if self.results else []},'timestamp':int(time.time()*1000)})+'\n')
        # #endregion
