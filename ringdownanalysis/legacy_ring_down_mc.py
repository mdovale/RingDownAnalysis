...
def estimate_freq_dft(x: np.ndarray, fs: float, window: str = "kaiser", kaiser_beta: float = 9.0) -> float:
    ...
    return estimate_freq_dft_optimized(
        x, fs, window=window, use_zeropad=False, pad_factor=1, lorentzian_points=7,
...
def monte_carlo_analysis(
    ...
    trial_args = [
        (i, f0, fs, N, A0, snr_db, Q, seed) for i in range(n_mc)
    ]
...
def plot_individual_results(results: dict):
    ...
    # NLS
    ax = axes[0]
    ax.hist(errors_nls, bins=30, density=True, alpha=0.7, edgecolor="black")
    ax.axvline(0, color="r", linestyle="--", label="Zero error")
    ax.axvline(crlb_std, color="g", linestyle="--", label=f"CRLB std = {crlb_std:.2e}")
    ax.axvline(-crlb_std, color="g", linestyle="--")
    ax.set_xlabel("Frequency error (Hz)")
    ax.set_ylabel("Probability density")
    ax.set_title(f"Nonlinear Least Squares (NLS)\nstd = {results['stats']['nls']['std']:.6e} Hz")
    ax.legend()
    ax.grid(True, alpha=0.3)
    
    # DFT
    ax = axes[1]
    ax.hist(errors_dft, bins=30, density=True, alpha=0.7, edgecolor="black")
    ax.axvline(0, color="r", linestyle="--", label="Zero error")
    ax.axvline(crlb_std, color="g", linestyle="--", label=f"CRLB std = {crlb_std:.2e}")
    ax.axvline(-crlb_std, color="g", linestyle="--")
    ax.set_xlabel("Frequency error (Hz)")
    ax.set_ylabel("Probability density")
    ax.set_title(f"Optimized DFT with Lorentzian Fitting\nstd = {results['stats']['dft']['std']:.6e} Hz")
    ax.legend()
    ax.grid(True, alpha=0.3)
...
def plot_aggregate_results(results: dict):
    ...
    ax.axvline(crlb_std, color="g", linestyle="--", linewidth=2, label=f"CRLB std = {crlb_std:.2e}")
    ax.axvline(-crlb_std, color="g", linestyle="--", linewidth=2)
    ax.set_xlabel("Frequency error (Hz)")
    ax.set_ylabel("Probability density")
    ...
    bp = ax.boxplot(data, tick_labels=["NLS", "DFT Lorentzian"], patch_artist=True)
    for patch in bp["boxes"]:
        patch.set_facecolor("lightblue")
        patch.set_alpha(0.7)
    ax.axhline(0, color="r", linestyle="--", linewidth=2, label="Zero error")
    ax.axhline(crlb_std, color="g", linestyle="--", linewidth=2, label=f"CRLB std = {crlb_std:.2e}")
    ax.axhline(-crlb_std, color="g", linestyle="--", linewidth=2)
...
def plot_performance_comparison(results: dict):
    ...
    ax.axhline(crlb_std, color="g", linestyle="--", linewidth=2, label=f"CRLB = {crlb_std:.2e}")
    ax.set_xticks(x_pos)
    ax.set_xticklabels(methods)
    ax.set_ylabel("Standard deviation (Hz)")
    ax.set_title("Frequency Estimation Uncertainty")
    ax.set_yscale("log")
    ax.legend()
    ax.grid(True, alpha=0.3, axis="y")
