...
- `notebooks/0.1_drifting-resonators.ipynb`: markdown says “Crop data to 3*tau” while code calls `analyzer.analyze_file(filepath)`.
- `notebooks/0.2_batch-analysis.ipynb`: markdown says “Crop data to 3×tau” while the helper calls `analyzer.analyze_file(filepath, max_tau_multiplier=1.0)`.
