...
\item Estimates quality factor $Q$ using both NLS and DFT+NLS sequential methods:
  \begin{itemize}
  \item \textbf{NLS}: Jointly estimates frequency and decay time, then computes $Q = \pi f \tau$.
  \item \textbf{DFT+NLS two-step approach}: (1) estimates frequency via DFT peak fitting with Lorentzian function, (2) estimates decay time via NLS with frequency fixed to the DFT result, then $Q = \pi f \tau$.
  \end{itemize}
