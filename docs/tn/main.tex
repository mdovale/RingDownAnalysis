...
\title{Frequency and Q-factor estimation of ring-down signals: nonlinear least squares and windowed discrete Fourier transform methods}
...
The CRLB in Eq.~\eqref{eq:crlb-ringdown} applies to any unbiased frequency estimator, including DFT-based methods. However, DFT peak estimators will typically not achieve this bound. The inefficiency arises due to a suboptimal use of the data, primarily due to:
\begin{itemize}
	\item windowing,
	\item discarding the phase information.
\end{itemize}
Even with an optimized flat-top window, the DFT remains a linear projection onto a fixed frequency grid rather than optimizing over continuous parameter space.
